...
const init = (option = {}) => {
  ...
  try {
    ...
    */
        ...
        commands.forEach((command) => {
          if (typeof (command.callback) === "function" && event.body !== undefined) {
            ...
            if (command.option.params === undefined)
              return console.error("[EagleBot]: No commands added, please add atleast 1 command");
    ...
    console.log("[EagleBot]: ", err.message);
  }
};

process.on("uncaughtException", (err) => console.log("[EagleBot]: ", err));
